...
#import "WUAlertSheet.h"
#import <Masonry/Masonry.h>
#import "WUPopViewCategories.h"

@interface WUAlertSheet()
...
- (instancetype)initWithTitle:(NSString *)title items:(NSArray *)items
{
    ...
    if ( self ) {
        ...
        WUAlertSheetConfig *config = [WUAlertSheetConfig shared];
        
        self.type = WUPopTypeSheet;
        ...
        for ( NSInteger i = 0 ; i < items.count; ++i ) {
            ...
            [btn mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.left.right.equalTo(self.buttonView).insets(UIEdgeInsetsMake(0, -WU_SPLIT_WIDTH, 0, -WU_SPLIT_WIDTH));
            ...
            [btn setBackgroundImage:[UIImage wu_imageWithColor:config.backgroundColor] forState:UIControlStateNormal];
            [btn setBackgroundImage:[UIImage wu_imageWithColor:config.backgroundColor] forState:UIControlStateDisabled];
            [btn setBackgroundImage:[UIImage wu_imageWithColor:config.itemPressedColor] forState:UIControlStateHighlighted];
        ...
        self.cancelButton = [UIButton wu_buttonWithTarget:self action:@selector(actionCancel)];
        ...
        [self.cancelButton setBackgroundImage:[UIImage wu_imageWithColor:config.backgroundColor] forState:UIControlStateNormal];
        [self.cancelButton setBackgroundImage:[UIImage wu_imageWithColor:config.itemPressedColor] forState:UIControlStateHighlighted];
...
@interface WUAlertSheetConfig()

@end

@implementation WUAlertSheetConfig

+ (WUAlertSheetConfig *)shared
{
    static WUAlertSheetConfig *config;
    static dispatch_once_t onceToken;
    
    dispatch_once(&onceToken, ^{
        
        config = [WUAlertSheetConfig new];
...
- (instancetype)init
{
    ...
    if ( self ) {
        ...
        self.titleColor         = [UIColor wu_colorWithHex:0x666666FF];
        self.splitColor         = [UIColor wu_colorWithHex:0xCCCCCCFF];
        
        self.itemNormalColor    = [UIColor wu_colorWithHex:0x333333FF];
        self.itemDisableColor   = [UIColor wu_colorWithHex:0xCCCCCCFF];
        self.itemHighlightColor = [UIColor wu_colorWithHex:0xE76153FF];
        self.itemPressedColor   = [UIColor wu_colorWithHex:0xEFEDE7FF];
