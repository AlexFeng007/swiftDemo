...
class ShopThreeAdPageCell: UITableViewCell {
    ...
    func setupView() {
        ...
        let space = (SCREEN_WIDTH - WH(113)*3 - WH(20))/2.0
        ...
        imgView.snp.makeConstraints({ (make) in
            make.height.width.equalTo(WH(113))
    ...
    fileprivate func getBannerDefaultImg() -> UIImage? {
        let view = UIView.init(frame: CGRect.init(x: 0, y: 0, width: WH(113), height: WH(113)))
        view.backgroundColor = RGBColor(0xF4F4F4)
        view.layer.masksToBounds = true
        view.layer.cornerRadius = WH(4.0)
        
        let imgview = UIImageView.init(frame: CGRect.init(x: 0, y: 0, width: WH(113), height: WH(113)))
