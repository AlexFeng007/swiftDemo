...
class ShopTwoAdPageCell: UITableViewCell {
    ...
    func setupView() {
        ...
        let adWidth = (SCREEN_WIDTH - WH(30))/2.0
        ...
        imgView.snp.makeConstraints({ (make) in
            make.width.equalTo(adWidth)
            make.height.equalTo((adWidth)*88/173.0)
    ...
    fileprivate func getBannerDefaultImg() -> UIImage? {
        let view = UIView.init(frame: CGRect.init(x: 0, y: 0, width: WH(174), height: WH(88)))
        view.backgroundColor = RGBColor(0xF4F4F4)
        view.layer.masksToBounds = true
        view.layer.cornerRadius = WH(4.0)
        
        let imgview = UIImageView.init(frame: CGRect.init(x: 0, y: 0, width: WH(174), height: WH(88)))
