//
//  ShopTimeCountView.swift
//  FKY
//
//  Created by 寒山 on 2020/4/8.
//  Copyright © 2020 yiyaowang. All rights reserved.
//

import UIKit

class ShopTimeCountView: UIView {
    ...
    // closure
       ...
       //时分秒背景
        ...
        override init(frame: CGRect) {
            ...
            NotificationCenter.default.addObserver(self, selector: #selector(stopTimerWhenHomeRefresh), name: NSNotification.Name(rawValue: FKYShopRefreshToStopTimers), object: nil)
        }
...
//            }
            ...
            lblColonLeft.snp.makeConstraints { (make) in
                make.centerY.equalTo(viewMinute.snp.centerY).offset(0)
                make.right.equalTo(viewMinute.snp.left).offset(WH(-2))
            ...
            lblColonRight.snp.makeConstraints { (make) in
                make.centerY.equalTo(viewMinute.snp.centerY).offset(0)
                make.left.equalTo(viewMinute.snp.right).offset(WH(2))
                make.width.equalTo(WH(8))
            }
            
            addSubview(lblHour)
            lblHour.snp.makeConstraints { (make) in
                make.right.equalTo(lblColonLeft.snp.left).offset(WH(-2))
    ...
    //
        ...
        @objc fileprivate func refreshHomeData() {
            ...
            count = 0
            stopCount()
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: FKYShopRefreshToStopTimers), object: self, userInfo: nil)
        }
        // MARK: - Notification
