...
class LivingListViewController: UIViewController {
    ...
    deinit {
        print("LivingListViewController deinit~!@")
    }
...
extension LivingListViewController: UITableViewDataSource,UITableViewDelegate {
...
extension LivingListViewController: UIScrollViewDelegate {
    //重新设置contentoffy
...
extension LivingListViewController{
    ...
    func getAllLiveInfoList(){
        showLoading()
        viewModel.getLiveActivityList("1"){ [weak self] (success, msg) in
            // 成功
            guard let strongSelf = self else {
                return
            }
