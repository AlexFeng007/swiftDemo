...
class ShopListVarietiesViewModel: ShopListViewModelInterface {
    ...
    fileprivate var logic: ShopListVarietiesLogic?
    ...
    // MARK: - life cycle
    init() {
        logic = ShopListVarietiesLogic.logic(with: HJNetworkManager.sharedInstance().generateOperationManger(withOwner: self)) as? ShopListVarietiesLogic
    }
}
...
extension ShopListVarietiesViewModel {
    ...
    func loadCacheData(finishedCallback: @escaping () -> ()) {
        logic?.fetchShopListTemplateCacheData({ [weak self] (templates, pageId, pageSize) in
            ...
            if let _ = templates as? [ShopListContainerProtocol & ShopListModelInterface]{
                        ...
                        if FKYCartModel.shareInstance().productArr.count > 0 {
                            for cartModel in FKYCartModel.shareInstance().productArr {
                                if let cartOfInfoModel = cartModel as? FKYCartOfInfoModel {
                                    if cartOfInfoModel.supplyId != nil &&  cartOfInfoModel.spuCode as String == product.productCode! && cartOfInfoModel.supplyId.intValue == Int(product.productSupplyId!) {
...
extension ShopListVarietiesViewModel {
    ...
    func updateShopCartItem(_ carId: String, count: Int, callback:@escaping (_ success: Bool, _ msg: String?)->()) {
        cartService.updateShopCart(forProduct: "\(carId)", quantity: count, allBuyNum: -1, success: { (mutiplyPage,aResponseObject) in
            // 成功
            callback(true, nil)
        }, failure: { (reason) in
            // 失败
    ...
    func addCart(_ vendorId: String, _ productId: String, promotionId: String, count: Int, callback:@escaping (_ msg: String?, _ data: AnyObject?)->()) {
        ...
        var paramArray = [Any]()
        paramJson["addType"] = "1"
        paramJson["sourceType"] = HomeString.MP_PZH_ADD_SOURCE_TYPE
        var params = [
            "supplyId": vendorId,
            "spuCode": productId,
            "productNum": "\(count)",
