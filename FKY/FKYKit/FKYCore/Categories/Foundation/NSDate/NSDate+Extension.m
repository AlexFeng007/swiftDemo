...
+ (NSString *)timeInfoWithDateMsgString:(NSString *)dateString {
    NSDate *date = [self dateWithString:dateString format:[self ymdHmsFormat]];
    ...
    if (time < 60) { // 小于一分钟
        ...
        // 同年
        return [NSString stringWithFormat:@"%lu/%lu",[date month],[date day]];
    }else{
        //跨年
        return [NSString stringWithFormat:@"%lu/%lu/%lu",[date year],[date month],[date day]];
    }
    return @"";
}
- (NSString *)ymdFormat {
