...
/**
 *  @brief  去除不显示的unicode字符
 *
 *  @return 去除不显示的unicode字符后的字符串
 */
- (NSString *)trimmingInvisibleUnicode
{
    return [self stringByReplacingOccurrencesOfString:@"\\p{Cf}" withString:@"" options:NSRegularExpressionSearch range:NSMakeRange(0, self.length)];
