...
- (NSArray*)allBasePropertyKeyPaths
{
    ...
    for (NSString* key in [self allPropertyKeys])
    {
        id obj = [self valueForKey:key];
        //NSLog(@"%@:%@", key, obj);
        NSArray *subKeys = [obj allBasePropertyKeyPathsWithBaseKeyPath:key];
        if	([subKeys count] > 0)
        {
...
- (NSArray*)allBasePropertyKeyPathsWithBaseKeyPath:(NSString*)baseKeyPath
{
    ...
    for (NSString* key in [self allPropertyKeys])
    {
        id obj = [self valueForKey:key];
        NSString *newBasePath = [baseKeyPath stringByAppendingFormat:@".%@", key];
        NSArray *subKeys = [obj allBasePropertyKeyPathsWithBaseKeyPath:newBasePath];
        if	([subKeys count] > 0)
        {
            [allKeyPaths addObjectsFromArray:subKeys];
        }
    ...
    for (NSString* key in keys)
    {
        if ([object isKindOfClass:[NSArray class]])
        {
            ...
            if (range.location != NSNotFound)
            {
                ...
                BOOL validIndex = (index < [(NSArray *)object count])&&(index >= 0);
                if (!validIndex)
                {
                    NSString* error = [NSString stringWithFormat:@"Index:%zd out of bounds for valueAtKeyPath:@\"%@\" for array:0x%p.count = %zd", index, keyPath, &object, [(NSArray *)object count]];
