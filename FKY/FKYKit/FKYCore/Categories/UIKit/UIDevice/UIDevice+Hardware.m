...
// keychain保存IDFV
+ (void)saveIdfvForDeviceId:(NSString *)uuid
{
    if (uuid && uuid.length > 0) {
        [SAMKeychain setPassword:uuid forService:@"idfv" account:@"admin"];
        return;
    }
    
    uuid = [UIDevice currentDevice].identifierForVendor.UUIDString;
    [SAMKeychain setPassword:(uuid ? uuid : @"") forService:@"idfv" account:@"admin"];
}

// keychain读取IDFV
+ (NSString *)readIdfvForDeviceId
{
    NSString *idfv = [SAMKeychain passwordForService:@"idfv" account:@"admin"];
    if (idfv && idfv.length > 0) {
        return idfv;
    }
    
    idfv = [UIDevice currentDevice].identifierForVendor.UUIDString;
    [self saveIdfvForDeviceId:idfv];
    return (idfv ? : @"");
}
