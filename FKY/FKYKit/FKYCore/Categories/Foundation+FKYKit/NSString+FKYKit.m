...
@implementation NSString (FKYKit)
...
// 判断是否为手机号...<老版>...<判断号段：13､14､15､17､18>
- (BOOL)isCellPhone
{
    //NSString *phonePattern = @"^[1][3578][0-9]{9}$";
    NSString *phonePattern = @"^[1][34578][0-9]{9}$";
    NSRegularExpression *phoneRegular = [[NSRegularExpression alloc] initWithPattern:phonePattern options:NSRegularExpressionCaseInsensitive error:nil];
    NSArray *PhoneResult = [phoneRegular matchesInString:self options:NSMatchingReportProgress range:NSMakeRange(0, self.length)];
    if (self.length != 11 || PhoneResult == nil || PhoneResult.count == 0) {
        return NO;
    }
    return YES;
}

// 判断是否为手机号...<新版>...<只判断开头为1的11位数字>
- (BOOL)isPhoneNumber
{
    NSString *phonePattern = @"^[1][0-9]{10}$";
    NSRegularExpression *phoneRegular = [[NSRegularExpression alloc] initWithPattern:phonePattern options:NSRegularExpressionCaseInsensitive error:nil];
    NSArray *PhoneResult = [phoneRegular matchesInString:self options:NSMatchingReportProgress range:NSMakeRange(0, self.length)];
    if (self.length != 11 || PhoneResult == nil || PhoneResult.count == 0) {
        return NO;
...
{
    if ([self hasPrefix:@"/"]) {
        return [GET_PIC_HOST() stringByAppendingString:self];
    }
    return [NSString stringWithFormat:@"%@/%@",GET_PIC_HOST(),self];
}
...
{
    __block NSString *preferString = @"";
    void(^formatteStringBlock)(NSString *spec,NSInteger value) = ^(NSString *spec,NSInteger value){
        preferString = [[NSString stringWithFormat:@"%zi%@",value,spec] stringByAppendingString:preferString];
    };
        formatteStringBlock(@"分",minute);
        if (hour || day) {
            formatteStringBlock(@"时",hour);
            if (day) {
                formatteStringBlock(@"天",day);
            }
        }
    return preferString;
}
...
+ (BOOL)validateOnlyChinese:(NSString *)text
{
    ...
    NSString *filtered = [NSString filterForChinese:text];
    return [text isEqualToString:filtered];
}
...
+ (NSString *)filterForChinese:(NSString *)text
{
    ...
    return [NSString filterCharactor:text withRegex:@"[^\u4e00-\u9fa5]"];
}
...
+ (NSString *)filterCharactor:(NSString *)text withRegex:(NSString *)regexStr
{
    ...
    NSString *result = [regex stringByReplacingMatchesInString:text options:NSMatchingReportCompletion range:NSMakeRange(0, text.length) withTemplate:@""];
    return result;
}
...
{
    NSString *codePattern = @"[0-9]{6}";
    NSRegularExpression *regular = [[NSRegularExpression alloc] initWithPattern:codePattern options:NSRegularExpressionCaseInsensitive error:nil];
    NSArray *result = [regular matchesInString:text options:NSMatchingReportProgress range:NSMakeRange(0, text.length)];
    if (text.length != 6 || result == nil || result.count == 0) {
        return NO;
    }
    return YES;
}
